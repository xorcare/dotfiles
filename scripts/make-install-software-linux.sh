...
[ -x "$(command -v 'apt')" ] && $SHELL "$DOTFILES_ROOT/scripts/make-install-software-linux-apt.sh"
[ -x "$(command -v 'yum')" ] && $SHELL "$DOTFILES_ROOT/scripts/make-install-software-linux-yum.sh"
